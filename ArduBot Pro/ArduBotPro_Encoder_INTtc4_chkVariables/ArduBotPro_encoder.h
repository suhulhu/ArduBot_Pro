...
void setupTimers()
{
  ...
  // timer TC4 counts up to CC0 value to generate 1ms interrupt,
  // this determines the frequency of the interrupt operation: Freq = 48Mhz/(N*CC0*Prescaler)
  REG_TC4_COUNT16_CC0 = 1499;                    // Set the CC0 (period) register to 1000 for 1MHz clock
  while (TC4->COUNT16.STATUS.bit.SYNCBUSY);     // Wait for synchronization
  ...
  REG_TC4_INTFLAG |= TC_INTFLAG_MC0;            // Clears the interrupt flag
