...
// setup 1ms interrupt using timer TC5, and clock source GCLK5
void setupTimers()
{
...
//  while (GCLK->STATUS.bit.SYNCBUSY);              // Wait for synchronization
  ...
  // set TC5 in 16 bits counting mode, Divide the GCLOCK signal by 16
  ...
  // timer TC5 counts up to CC0 value to generate 1ms interrupt,
  // this determines the frequency of the interrupt operation: Freq = 48Mhz/(N*CC0*Prescaler)
  REG_TC5_COUNT16_CC0 = 1499;                    // Set the CC0 (period) register to 1000 for 1MHz clock
  while (TC5->COUNT16.STATUS.bit.SYNCBUSY);     // Wait for synchronization

  NVIC_SetPriority(TC5_IRQn, 0);                // Set the Nested Vector Interrupt Controller (NVIC) priority for TC5 to 3 (0 highest)
  NVIC_EnableIRQ(TC5_IRQn);                     // Connect TC5 to Nested Vector Interrupt Controller (NVIC)
  REG_TC5_INTFLAG |= TC_INTFLAG_MC0;            // Clears the interrupt flag
